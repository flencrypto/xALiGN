...
function IntelPanel({ intel, accountId, created }: { intel: SwoopIntel; accountId: number; created: boolean }) {
  return (
    <div className="mt-4 space-y-3 text-sm">
      {/* Status badge */}
      <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${created ? 'bg-green-500/10 border-green-500/30 text-green-300' : 'bg-blue-500/10 border-blue-500/30 text-blue-300'}`}>
      ...
      {intel.intel_summary && (
        <div className="bg-slate-700/50 rounded-lg p-3">
          <p className="text-slate-400 text-xs font-medium uppercase tracking-wider mb-1">📊 Intel Summary</p>
          <p className="text-white">{intel.intel_summary}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="bg-slate-700/50 rounded-lg p-3 space-y-1">
          <p className="text-slate-400 text-xs font-medium uppercase tracking-wider mb-2">🏢 Company</p>
          {intel.company_name && <p className="text-white font-semibold">{intel.company_name}</p>}
          {intel.type && <p className="text-slate-300"><span className="text-slate-400">Type:</span> {intel.type}</p>}
          {intel.location && <p className="text-slate-300"><span className="text-slate-400">Location:</span> {intel.location}</p>}
          {intel.stock_ticker && (
            <p className="text-slate-300 font-mono">
              <span className="text-slate-400">Stock:</span>{' '}
              <span className="text-blue-400">{intel.stock_ticker}</span>
            </p>
          )}
          {intel.tags && intel.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 pt-1">
              {intel.tags.map((tag) => (
                <span key={tag} className="px-2 py-0.5 bg-slate-600 rounded text-xs text-slate-300">{tag}</span>
              ))}
            </div>
          )}
        </div>
        ...
        {intel.suggested_touchpoint && (
          <div className="bg-blue-600/10 border border-blue-500/20 rounded-lg p-3">
            <p className="text-blue-400 text-xs font-medium uppercase tracking-wider mb-2">💬 Suggested Touchpoint</p>
            <p className="text-slate-200 italic text-xs leading-relaxed">&ldquo;{intel.suggested_touchpoint}&rdquo;</p>
          </div>
        )}
      </div>
      ...
      {intel.key_personnel && intel.key_personnel.length > 0 && (
        <div className="bg-slate-700/50 rounded-lg p-3">
          <p className="text-slate-400 text-xs font-medium uppercase tracking-wider mb-2">👤 Key Personnel</p>
            ...
            {intel.key_personnel.map((p, i) => (
              <li key={i} className="flex items-start gap-2">
                ...
                <div>
                  ...
                  {p.role && <span className="text-slate-400 text-xs ml-1">– {p.role}</span>}
                  <div className="flex gap-3 mt-0.5">
                    ...
                    {(() => {
                      ...
                      return url ? (
                        <a href={url} target="_blank" rel="noopener noreferrer"
                          className="text-slate-300 text-xs hover:underline">✕ {p.x_handle}</a>
                      ) : null;
                    })()}
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        {intel.triggers && intel.triggers.length > 0 && (
          <div className="bg-slate-700/50 rounded-lg p-3">
            <p className="text-slate-400 text-xs font-medium uppercase tracking-wider mb-2">⚡ Trigger Signals</p>
            <ul className="space-y-1">
              {intel.triggers.map((t, i) => (
                <li key={i} className="text-slate-300 text-xs flex gap-1.5">
                  <span className="text-yellow-400 flex-shrink-0">→</span>{t}
                </li>
              ))}
            </ul>
          </div>
        ...
        {intel.recent_news && intel.recent_news.length > 0 && (
          <div className="bg-slate-700/50 rounded-lg p-3">
            <p className="text-slate-400 text-xs font-medium uppercase tracking-wider mb-2">📰 Recent News</p>
            <ul className="space-y-1">
              {intel.recent_news.map((n, i) => (
                <li key={i} className="text-slate-300 text-xs flex gap-1.5">
                  <span className="text-slate-500 flex-shrink-0">•</span>{n}
                </li>
              ))}
            </ul>
          </div>
...
export default function WebsiteSwoop({ onAccountCreated }: Props) {
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
      <h2 className="text-white font-semibold mb-1">🌐 Website Swoop</h2>
      ...
      <form onSubmit={handleSwoop} className="flex gap-3">
        <input
          ...
          required
          disabled={loading}
          className="flex-1 bg-slate-700 border border-slate-600 rounded-lg px-4 py-2 text-white text-sm placeholder-slate-400 focus:outline-none focus:border-blue-500 disabled:opacity-60"
        />
        <button
          type="submit"
      ...
      {error && (
        <p className="text-red-400 text-sm mt-3 bg-red-500/10 border border-red-500/30 rounded-lg px-3 py-2">
          {error}
        </p>
      )}
