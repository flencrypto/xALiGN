...
function scoreColor(score?: number) {
  if (!score) return 'bg-surface text-text-muted';
        ...
        {selected && (
          <div className="w-80 xl:w-96 border-l border-border-subtle bg-surface overflow-auto p-5 flex-shrink-0">
            ...
            <div className="space-y-2 text-sm mb-5">
              ...
              <p className="text-text-muted">Stage: <span className={`px-2 py-0.5 rounded text-xs border ${STAGE_COLORS[selected.stage]}`}>{selected.stage}</span></p>
              ...
              {selected.probability != null && <p className="text-text-muted">Probability: <span className="text-text-main">{selected.probability}%</span></p>}
            </div>
            ...
            <div className="bg-surface/50 rounded-xl p-4 mb-4">
              ...
              {qualification ? (
                <>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      {qualification.recommendation && <p className={`text-sm font-semibold ${qualification.score >= 60 ? 'text-success' : 'text-danger'}`}>{qualification.recommendation}</p>}
                    </div>
                  </div>
                ...
                <p className="text-text-muted text-xs mb-3">No qualification data yet. Run the qualification to get a Go/No-Go score.</p>
              )}
              <button onClick={runQualification} disabled={qualifying} className="w-full mt-2 bg-primary hover:bg-primary-dark text-text-main py-2 rounded-lg text-sm font-medium disabled:opacity-50 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-surface border border-border-subtle rounded-xl w-full max-w-md p-6">
            ...
            <form onSubmit={handleCreate} className="space-y-3">
              ...
              <input placeholder="Probability (%)" type="number" min="0" max="100" value={form.probability} onChange={(e) => setForm({ ...form, probability: e.target.value })} className="w-full bg-background border border-border-subtle text-text-main rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-500" />
